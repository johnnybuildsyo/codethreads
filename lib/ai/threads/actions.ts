...
export async function generateThreadIdeas(codeChanges: string) {
  ...
  const prompt = `You are an API that is used to generate ideas for post threads from code changes. Analyze the following code changes as a whole then generate 3 or more ideas that would make for compelling threads to post on social media. Come up with ideas related specifically to the OVERALL code changes in their entirety. Do not include file names in your response: ${codeChanges}`;

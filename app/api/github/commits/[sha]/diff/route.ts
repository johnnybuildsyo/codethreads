...
export async function GET(request: Request, { params }: { params: { sha: string } }) {
  ...
  const files = (commit.files || []).map((file: GitHubFile) => ({
    ...
    oldValue: file.status === 'added' ? '' : file.patch?.split('\n').filter(line => line.startsWith('-')).map(line => line.slice(1)).join('\n') || '',
    newValue: file.status === 'removed' ? '' : file.patch?.split('\n').filter(line => line.startsWith('+')).map(line => line.slice(1)).join('\n') || ''
  }))
